...
To make official [Typography](https://tailwindcss-typography.vercel.app/) plugin available in your project, first you need to install package.
...
<Alert>Recently I created [Clampography](https://www.npmjs.com/package/clampography) - a fluid typography and theming plugin for Tailwind CSS v4. It automatically generates a fluid type scale using CSS clamp() functions.</Alert>

Instead of the official plugin, you can use Clampography. It restores typography defaults and scales smoothly between mobile and desktop sizes.
...
Now you just need to apply the theme to your **body** tag. Unlike the official plugin, Clampography uses `:where()` selectors, meaning it has zero specificity and doesn't require a `prose` class wrapper.
